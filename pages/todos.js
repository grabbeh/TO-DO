        ...
        <div className='flex'>
          <label className='flex items-center space-x-3'>
            <input
              ...
              checked={completed}
              onChange={handleChange}
              className='cursor-pointer form-checkbox h-6 w-6 border hover:form-checkbox border-gray-300 rounded-md checked:color-green-500 checked:bg-blue-600 checked:border-transparent focus:outline-none'
            />
            <span
              className={`text-gray-900 ${completed &&
                'line-through'} text-xl font-medium`}
            >
              {todo.text}
            </span>
          </label>
          {!todo.deleted && (
            <div
              className='ml-4 h-6 w-6 text-gray-500 hover:text-black cursor-pointer'
              onClick={() => {
                let updated = { ...todo, deleted: true }
                addToDo({
                  variables: {
                    todo: { ...updated }
                  }
